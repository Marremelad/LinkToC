...
parser.add_argument("mode", default="", help="[l]everera or [r]ensa")
parser.add_argument(
    "-dc", "--dockercompose", default="", help="Path to docker-compose file"
)
parser.add_argument(
    "--debug", action="store_true", help="Flag for debug run, dont leverera/rensa"
)
...
# Gets endpointID based on ENDPOINT_ALIAS
def getEndpointData() -> tuple:
    global response, request_endpoint, request_data, request_header
    getEnvironmentVars(["ENDPOINT_ALIAS"])

    # Get which endpoint ID we are deploying to
    print(f"\nGetting ENDPOINT_ID and SWARM_ID for endpoint with name: {ENDPOINT_ALIAS}")
    setAPIEndpoint("endpoints")
    setRequestParameters(ENDPOINT_ALIAS)
...
# Checks if STACK_NAMN is already deployed, returns empty string if STACK_NAMN doesnt exist
...
def isStackDeployed() -> str:
    ...
    # Check if STACK_NAMN exists
    for stack in stack_data:
        if stack["Name"] == STACK_NAMN:
            print(f"Stack with name {STACK_NAMN} exists with ID: {stack['Id']}")
            return stack["Id"]
    print(f"Stack with name {STACK_NAMN} doesn't exist")
    return stack_id
...
def deploy():
    ...
    # Check if STACK_NAMN is present
    ...
    if stack_id:  # Not empty string, stack exists
        ...
        request_data = {"Name": STACK_NAMN, "SwarmID": swarm_id}
...
def teardown():
    ...
    if stack_id:  # Not empty string, is deployed
        ...
        print(f"Deletion of stack {STACK_NAMN} successful")
    else:
        sys.exit(1)
...
# Get all relevant pipeline variables from OS environment for leverera/rensa jobs
getEnvironmentVars(
    [
        "CI_PROJECT_NAMESPACE",
        "CI_PROJECT_NAME",
